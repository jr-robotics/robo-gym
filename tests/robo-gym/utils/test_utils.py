...
@pytest.mark.parametrize(
    "x, y, center_x, center_y, radius, expected_result", test_inside_circle
)
...
def test_cartesian_to_polar_2d_default_origin():
    ...
    polar_r, polar_theta = utils.cartesian_to_polar_2d(
        x_target=target_x, y_target=target_y
    )
...
def test_cartesian_to_polar_2d_set_origin():
    ...
    polar_r, polar_theta = utils.cartesian_to_polar_2d(
        x_target=target_x, y_target=target_y, x_origin=origin_x, y_origin=origin_y
    )
...
def test_cartesian_to_polar_2d_equal_points(p1, p2, expected_r, expected_theta):
    ...
    polar_r, polar_theta = utils.cartesian_to_polar_2d(
        x_target=p1[0], y_target=p1[1], x_origin=p2[0], y_origin=p2[1]
    )
...
def test_cartesian_to_polar_3d():
    ...
    r, theta, phi = utils.cartesian_to_polar_3d(
...
def test_downsample_list_to_len(target_length):
    ...
    downsampled_list = utils.downsample_list_to_len(
        data=sample_list, output_len=target_length
    )
...
def test_translation_change_reference_frame():
    ...
    assert (
        utils.change_reference_frame(point, translation, quaternion) == [-6, 9, 1]
    ).all()
...
def test_rotation_change_reference_frame():
    ...
    assert np.allclose(
        a=utils.change_reference_frame(point, translation, quaternion),
        b=[0.250, -0.256, 0.890],
        atol=0.001,
    )


def test_quat_from_rpy():
    roll = 0.25
    pitch = 0.5
    yaw = 0.75
...
def test_rot_diff():
    ...
    r3 = utils.quat_from_euler_xyz(0.0, 0.5, 0.0)

    diff_1_2 = utils.rotation_error_magnitude(r1, r2)
    diff_1_3 = utils.rotation_error_magnitude(r1, r3)
    diff_2_3 = utils.rotation_error_magnitude(r2, r3)

    diff_3_3 = utils.rotation_error_magnitude(r3, r3)

    assert math.isclose(diff_3_3, 0, abs_tol=0.000001)
