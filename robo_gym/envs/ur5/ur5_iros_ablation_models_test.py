...
DEBUG = True
class AblationTestEnv(IrosEnv01UR5):
    ep_n = 0 
...
class AblationTestEnvDoF5(AblationTestEnv):
    def _get_action_space(self):
        return spaces.Box(low=np.full((5), -1.0), high=np.full((5), 1.0), dtype=np.float32)

class AblationTestEnvSim(AblationTestEnv, Simulation):
    ...
    def __init__(self, ip=None, lower_bound_port=None, upper_bound_port=None, gui=False, **kwargs):
        ...
        AblationTestEnv.__init__(self, rs_address=self.robot_server_ip, **kwargs)

class AblationTestEnvDoF5Sim(AblationTestEnvDoF5, Simulation):
    cmd = "roslaunch ur_robot_server ur5_sim_robot_server.launch \
        world_name:=box100.world \
        yaw:=-0.78\
        reference_frame:=world \
    ...
    def __init__(self, ip=None, lower_bound_port=None, upper_bound_port=None, gui=False, **kwargs):
        Simulation.__init__(self, self.cmd, ip, lower_bound_port, upper_bound_port, gui, **kwargs)
        AblationTestEnvDoF5.__init__(self, rs_address=self.robot_server_ip, **kwargs)

class AblationTestEnvDoF5Rob(AblationTestEnvDoF5):
    real_robot = True
