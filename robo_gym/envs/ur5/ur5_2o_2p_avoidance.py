...
class ObstacleAvoidance2Box2PointsUR5(MovingBoxTargetUR5):
    ...
    def _get_observation_space(self):
        ...
        # Object 2 coordinates (with respect to forearm frame) range
        object2_forearm_range = np.full(3, np.inf)

        # Definition of environment observation_space
        max_obs = np.concatenate((target_range, max_joint_positions, max_delta_start_positions, object2_range, target_forearm_range, object2_forearm_range))
        min_obs = np.concatenate((-target_range, min_joint_positions, min_delta_start_positions, -object2_range, -target_forearm_range, -object2_forearm_range))
    ...
    def _robot_server_state_to_env_state(self, rs_state):
        ...
        forearm_to_base_translation = rs_state[29:32]
        forearm_to_base_quaternion = rs_state[32:36]
        ...
        object2_coord_forearm_frame = utils.change_reference_frame(object2_coord,base_to_forearm_translation,base_to_forearm_quaternion)
        object2_polar_forearm = utils.cartesian_to_polar_3d(object2_coord_forearm_frame)


        # Compose environment state
        state = np.concatenate((target_polar, ur_j_pos_norm, delta_joints, object2_polar,target_polar_forearm,object2_polar_forearm))
    ...
    def _get_robot_server_state_len(self):
        ...
        rs_state = target + ur_j_pos + ur_j_vel + ee_to_base_transform + ur_collision + object_2 + forearm_to_base_transform

        return len(rs_state)

class ObstacleAvoidance2Box2PointsUR5DoF3(ObstacleAvoidance2Box2PointsUR5):
    def _get_action_space(self):
        return spaces.Box(low=np.full((3), -1.0), high=np.full((3), 1.0), dtype=np.float32)

class ObstacleAvoidance2Box2PointsUR5DoF5(ObstacleAvoidance2Box2PointsUR5):
    def _get_action_space(self):
        return spaces.Box(low=np.full((5), -1.0), high=np.full((5), 1.0), dtype=np.float32)


class ObstacleAvoidance2Box2PointsUR5Sim(ObstacleAvoidance2Box2PointsUR5, Simulation):
    ...
    def __init__(self, ip=None, lower_bound_port=None, upper_bound_port=None, gui=False, **kwargs):
        ...
        ObstacleAvoidance2Box2PointsUR5.__init__(self, rs_address=self.robot_server_ip, **kwargs)

class ObstacleAvoidance2Box2PointsUR5Rob(ObstacleAvoidance2Box2PointsUR5):
    real_robot = True

# roslaunch ur_robot_server ur5_real_robot_server.launch  gui:=true reference_frame:=base max_velocity_scale_factor:=0.2 action_cycle_rate:=20 target_mode:=2moving2points

class ObstacleAvoidance2Box2PointsUR5DoF3Sim(ObstacleAvoidance2Box2PointsUR5DoF3, Simulation):
    cmd = "roslaunch ur_robot_server ur5_sim_robot_server.launch \
        world_name:=2_box100.world \
        yaw:=-0.78\
        reference_frame:=world \
    ...
    def __init__(self, ip=None, lower_bound_port=None, upper_bound_port=None, gui=False, **kwargs):
        Simulation.__init__(self, self.cmd, ip, lower_bound_port, upper_bound_port, gui, **kwargs)
        ObstacleAvoidance2Box2PointsUR5DoF3.__init__(self, rs_address=self.robot_server_ip, **kwargs)

class ObstacleAvoidance2Box2PointsUR5DoF3Rob(ObstacleAvoidance2Box2PointsUR5DoF3):
    real_robot = True

# roslaunch ur_robot_server ur5_real_robot_server.launch  gui:=true reference_frame:=base max_velocity_scale_factor:=0.2 action_cycle_rate:=20 target_mode:=2moving2points

class ObstacleAvoidance2Box2PointsUR5DoF5Sim(ObstacleAvoidance2Box2PointsUR5DoF5, Simulation):
    cmd = "roslaunch ur_robot_server ur5_sim_robot_server.launch \
        world_name:=2_box100.world \
        yaw:=-0.78\
        reference_frame:=world \
    ...
    def __init__(self, ip=None, lower_bound_port=None, upper_bound_port=None, gui=False, **kwargs):
        Simulation.__init__(self, self.cmd, ip, lower_bound_port, upper_bound_port, gui, **kwargs)
        ObstacleAvoidance2Box2PointsUR5DoF5.__init__(self, rs_address=self.robot_server_ip, **kwargs)

class ObstacleAvoidance2Box2PointsUR5DoF5Rob(ObstacleAvoidance2Box2PointsUR5DoF5):
    real_robot = True
