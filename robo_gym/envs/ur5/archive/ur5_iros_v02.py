...
from robo_gym.envs.ur5.ur5_iros_v01 import IrosEnv01UR5
...
class IrosEnv02UR5FixObstacleTrajDoF5(IrosEnv02UR5FixObstacleTraj):
    def _get_action_space(self):
        return spaces.Box(low=np.full((5), -1.0), high=np.full((5), 1.0), dtype=np.float32)

class IrosEnv02UR5FixObstacleTrajDoF5Sim(IrosEnv02UR5FixObstacleTrajDoF5, Simulation):
    cmd = "roslaunch ur_robot_server ur5_sim_robot_server.launch \
        world_name:=box100.world \
        yaw:=-0.78\
        reference_frame:=world \
    ...
    def __init__(self, ip=None, lower_bound_port=None, upper_bound_port=None, gui=False, **kwargs):
        Simulation.__init__(self, self.cmd, ip, lower_bound_port, upper_bound_port, gui, **kwargs)
        IrosEnv02UR5FixObstacleTrajDoF5.__init__(self, rs_address=self.robot_server_ip, **kwargs)

class IrosEnv02UR5FixObstacleTrajDoF5Rob(IrosEnv02UR5FixObstacleTrajDoF5):
    real_robot = True

# roslaunch ur_robot_server ur5_real_robot_server.launch  gui:=true reference_frame:=base max_velocity_scale_factor:=0.2 action_cycle_rate:=20 target_mode:=1moving2points objects_controller:=true publish_objects_tf:=true n_objects:=1.0 object_0_frame:=target
