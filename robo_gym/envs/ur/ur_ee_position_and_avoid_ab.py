...
class ReachAndAvoidABUR(URBaseEnv):
    ...
    def reset(self, joint_positions = JOINT_POSITIONS, ee_target_pose = None, randomize_start=False, continue_on_success=False, fixed_object_position = None) -> np.ndarray:
        ...
        if fixed_object_position:
            ...
            delta_x = float(np.random.default_rng().uniform(low=0, high=0.1))
            delta_y = float(np.random.default_rng().uniform(low=0, high=0.1))
            delta_z = float(np.random.default_rng().uniform(low=0, high=0.1))
            fixed_object_position = [ee_target_pose[0]+delta_x, ee_target_pose[1]+delta_y, ee_target_pose[2]+delta_z]

        # Set initial state of the Robot Server
        state_msg = self._set_initial_robot_server_state(rs_state, ee_target_pose, fixed_object_position)
...
class ReachAndAvoidABURSim(ReachAndAvoidABUR, Simulation):
    ...
    def __init__(self, ip=None, lower_bound_port=None, upper_bound_port=None, gui=False, ur_model='ur5', **kwargs):
        ...
        ReachAndAvoidABUR.__init__(self, rs_address=self.robot_server_ip, ur_model=ur_model, **kwargs)

class ReachAndAvoidABURRob(ReachAndAvoidABUR):
    real_robot = True 
