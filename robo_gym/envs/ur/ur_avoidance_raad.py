...
class AvoidanceRaad2022UR(URBaseAvoidanceEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        file_path = os.path.join(
            os.path.dirname(__file__), "robot_trajectories", file_name
        )
    ...
    def _set_initial_robot_server_state(
        ...
        float_params = {
            ...
            "object_0_z_min": 0.1,
            "object_0_z_max": 1.0,
            "object_0_n_points": 10,
            "object_0_n_sampling_points": n_sampling_points,
    ...
    def reward(self, rs_state, action) -> Tuple[float, bool, dict]:
        ...
        distance_to_elbow = np.linalg.norm(
            np.array(target_coord) - np.array(forearm_coord)
        )
        ...
        if abs(action).sum() <= action.size:
            reward += (
                action_usage_weight
                * (1 - (np.square(action).sum() / action.size))
                * (1 / 1000)
            )
        ...
        if (distance_to_ee < MINIMUM_DISTANCE) or (
            distance_to_elbow < MINIMUM_DISTANCE
        ):
    ...
    def _robot_server_state_to_env_state(self, rs_state) -> np.ndarray:
        ...
        trajectory_joint_position = self.ur.normalize_joint_values(
    ...
    def _get_joint_positions(self) -> np.ndarray:
        ...
        if self.elapsed_steps_in_current_state < len(self.trajectory[self.state_n]):
            joint_positions = copy.deepcopy(
                self.trajectory[self.state_n][self.elapsed_steps_in_current_state]
            )
