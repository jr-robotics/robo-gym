...
class URBaseAvoidanceEnv(URBaseEnv):
    ...
    def _set_initial_robot_server_state(
        self, rs_state, fixed_object_position
    ) -> robot_server_pb2.State:
        string_params = {}
        float_params = {}
        state = {}
    ...
    def _robot_server_state_to_env_state(self, rs_state) -> np.ndarray:
        ...
        object_coord_ee_frame = utils.change_reference_frame(
            object_coord, ref_frame_to_ee_translation, ref_frame_to_ee_quaternion
        )
        ...
        starting_joints = self.ur.normalize_joint_values(
            self._get_joint_positions_as_array()
        )
        ...
        # t' = - R^-1 * t
        ref_frame_to_forearm_translation = -ref_frame_to_forearm_rotation.apply(
            forearm_to_ref_frame_translation
        )

        object_coord_forearm_frame = utils.change_reference_frame(
            object_coord,
            ref_frame_to_forearm_translation,
        ...
        if self.include_polar_to_elbow:
            state = np.concatenate(
                (object_polar, joint_positions, delta_joints, object_polar_forearm)
            )
        else:
            state = np.concatenate(
                (object_polar, joint_positions, delta_joints, np.zeros(3))
            )
